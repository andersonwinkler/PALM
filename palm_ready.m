...
% If a mask was explicitly provided, check that it has same affine matrix
% as the imput data
if ~isempty(maskstruct) && ...
        isfield(maskstruct,'affine') && isfield(Ytmp,'affine') && ...
        ~isscalar(maskstruct.affine) && ~isscalar(Ytmp.affine)
    if any(Ytmp.affine(:) ~= maskstruct.affine(:))
        Yaffine = Ytmp.affine';
        Maffine = maskstruct.affine';
        error('%s', sprintf([...
            ...
            '    %f %f %f %f\n'       ...
            '    %f %f %f %f ]\n'],   ...
            Ytmp.filename,Yaffine(:),maskstruct.filename,Maffine(:)))
    end
end
...
if strcmp(Ytmp.readwith,'nifticlass') && ndims(Ytmp.data) == 4
    if isempty(maskstruct)
        ...
        maskstruct = palm_maskstruct(tmpmsk(:)',Ytmp.readwith,Ytmp.extra,Ytmp.affine,Ytmp.size);
    else
...
if ndims(Ytmp.data) == 2 %#ok<ISMAT>
    ...
    % dimension was shifted earlier to be the last
    if opts.transposedata || strcmpi(Ytmp.readwith,'cifti-matlab')
        Ytmp.data = Ytmp.data';
    end
...
if strcmp(Ytmp.readwith,'nifticlass')
    ...
    if isempty(maskstruct)
        maskstruct = palm_maskstruct(maskydat,Ytmp.readwith,Ytmp.extra,Ytmp.affine,Ytmp.size);
    else
