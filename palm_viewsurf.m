...
%               If the data being read is an annotation file, you can
%               specify 'annot' to use the embedded colour table.
...
%              - 'Interp'
%              - 'Faceted'
% FaceColor  : How faces should be coloured:
%              - 'Flat': uses the colour of the first vertex
%              - 'Interp': interpolates across vertices (default)
...
opts.facecolor  = 'interp';  % {'flat', 'interp'}
opts.colourbar  = true;
opts.inflated   = false;
...
if ~ any(strcmpi(opts.facecolor,{'flat','interp'}))
    error('Unknown FaceColor style: %s',opts.facecolor);
end
...
C = (min(srf{3}.data.vtx) + max(srf{3}.data.vtx))/2;
for s = 1:3
...
switch opts.layout
    ...
    case 'simple'
        ...
        pathandle = trisurf(...
            ...
            dat{2}.data,...
            'EdgeColor','None','FaceColor',opts.facecolor);
        view(-245,20);
        useopts(opts,fighandle,pathandle);
    ...
    case 'cardinal'
        ...
        pathandle = trisurf(srf{2}.data.fac,...
            srf{2}.data.vtx(:,1),srf{2}.data.vtx(:,2),srf{2}.data.vtx(:,3),...
            dat{2}.data,'EdgeColor','None','FaceColor',opts.facecolor);
        view(-90,0);
        useopts(opts,fighandle,pathandle);
...
function useopts(opts,fighandle,pathandle) % ==================================================
daspect([1 1 1]);
box off
...
set(pathandle,'FaceColor',opts.facecolor);